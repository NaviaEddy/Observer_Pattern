...
void AJefeEspecial1::Transformar()
{
    ...
    if (Tiempo.Equals("por el dia"))
    {
        ...
        GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString::Printf(TEXT("Es %s, así que JEFE ESPECIAL 1 hace un desayuno frances"), *Tiempo));
    }
    else if (Tiempo.Equals("por mediodia"))
    {
        //Ejecuta la rutina del mediodia
        GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString::Printf(TEXT("Es %s, así que la mano izquierda del JEFE ESPECIAL 1 empieza a caerse..."), *Tiempo));
    }
